# Fixora AWS Infrastructure Architecture
...
| `fixora-backend` (user) | Render backend | Upload to S3, run ECS tasks, read logs |
...
- User triggers code validation through the Fixora frontend (e.g., creating a PR or testing fixes)
...
- Bundle is uploaded to S3 bucket: `s3://fixora-sandbox-code-<account-id>/jobs/<job-id>/`
...
> [!NOTE]
> We created a dedicated IAM user `fixora-admin` for setup to avoid using root credentials.
...
aws s3 mb s3://fixora-sandbox-code-<account-id>

# Enable server-side encryption
aws s3api put-bucket-encryption \
    --bucket fixora-sandbox-code-<account-id> \
...
| `AWS_S3_BUCKET` | `fixora-sandbox-code-<account>` | Code transfer bucket |
...
aws s3 rb s3://fixora-sandbox-code-<account-id> --force
aws iam delete-user-policy --user-name fixora-backend --policy-name backend-sandbox-access
aws iam delete-access-key --user-name fixora-backend --access-key-id <key-id>
aws iam delete-user --user-name fixora-backend
...
| Check S3 bucket | `aws s3 ls s3://fixora-sandbox-code-<account>/` |
